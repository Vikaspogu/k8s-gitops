...
1. Create base64 encoded Redis configuration
    ...
    ```sh
    echo -n '{"db":15,"sentinels":[{"host":"redis-node-0.redis-headless.data.svc.cluster.local","port":26379},{"host":"redis-node-1.redis-headless.data.svc.cluster.local","port":26379},{"host":"redis-node-2.redis-headless.data.svc.cluster.local","port":26379}],"name":"redis-master"}' \
        | base64 -w 0
    ```
...
6. Apply the bucket policies
    ...
    ```sh
    mc admin policy add minio outline-private ~/.mc/outline-user-policy.json
    ```
...
8. Associate public policy with the bucket
    ...
    ```sh
    mc anonymous set-json ~/.mc/bucket-policy.json minio/outline
    ```
